...
FROM\"Outage\" WHERE(\"Outages\".\"Planned Start Date (Local)\" >=timestampadd(sql_tsi_day,-1, current_date) ) 
AND( (\"Studies\".\"Interface Limitation Flag\" = 'Yes'))
...
AND (\"Outages\".\"Outage Status Description\" <> 'Implemented')
AND (\"Outage\".\"Outages\".\"Cause Type\" <> 'MODELING')
AND (\"Outages\".\"Outage Requested Equipment Text\" LIKE '%OOS%')
AND (\"Outages\".\"Outage Priority Description\" LIKE '%Long Term%')
OR (\"Outages\".\"Planned Start Date (Local)\" >=timestampadd(sql_tsi_day,-1, current_date) ) 
AND( (\"Studies\".\"Interface Limitation Flag\" = 'Yes'))
AND (\"Outages\".\"Outage Status Description\" <> 'Cancelled')
AND (\"Outages\".\"Outage Status Description\" <> 'Denied')
AND (\"Outages\".\"Outage Status Description\" <> 'Completed')
...
AND (\"Outage\".\"Outages\".\"Cause Type\" <> 'MODELING')
AND (\"Outages\".\"Outage Requested Equipment Text\" LIKE '%OOS%')
AND(\"Outages\".\"Outage Priority Description\" LIKE '%Planned%')
...
Implemented <- read_excel("\\\\iso-ne.com\\shares\\performance_auditing\\OverlappingOutages\\implemented.xlsx")


OVERLAP<- merge(OVERLAP,Implemented,all = TRUE)
...
chartColors <- c("Potential" = "darkviolet","Implemented" = "gray54", "Approved" = "Blue", "Interim Approved" = "green4", "Preliminary" = "cyan1", "Negotiate" = "tan4", "Study" = "sandybrown", "Submitted" = "Red", "Cancelled" = "Maroon")
